...
public class SelectArticleVisit extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!temp.equals(loginID)){
			...
			if (checkPageFrame(pageContain)){
				page = service.selectVisitArticle(Integer.parseInt(pageIndex), Integer.parseInt(pageContain), 5);
			}else{
				page = service.selectVisitArticle(Integer.parseInt(pageIndex), 10, 5);
			}
			if(page == null){
				msg.setMessageCode("-300");
				msg.setMessageDetail("访问记录获取失败");
				response.getWriter().write(mapper.writeValueAsString(msg));
			}else{
